...
class TestMicrofinanceRecovery(unittest.TestCase):
    ...
    def test_interests(self):
        recovery = create_test_recovery(
            posting_date="2017-09-17", total_interests=10000.0, principal_amount=5000.0
        )
    ...
    def test_interests_partial(self):
        recovery = create_test_recovery(total_interests=7000.0, principal_amount=5000.0)
        self.assertEqual(recovery.periods[0].billed_amount, 10000)
        self.assertEqual(recovery.periods[0].outstanding_amount, 10000)
        ...
        to_check = {"billed_amount": 10000, "paid_amount": 7000}
        for k, v in to_check.items():
            self.assertEqual(v, period.get(k))

    def test_interests_multiple(self):
        recovery = create_test_recovery(
            total_interests=17000.0, principal_amount=5000.0
        )
        period = frappe.get_doc(
    ...
    def test_interests_multiple_paid_amounts(self):
        create_test_recovery(total_interests=17000.0, principal_amount=5000.0)
        exp_amounts = dict(
            (d[0], d)
            for d in [["_Test Loan 1/2017-08", 10000], ["_Test Loan 1/2017-09", 7000]]
        )
        ...
        for per in periods:
            self.assertEquals(exp_amounts[per.get("name")][0], per.get("name"))
            self.assertEquals(exp_amounts[per.get("name")][1], per.get("paid_amount"))

    def test_interests_multiple_posting_dates(self):
        create_test_recovery(
            posting_date="2017-09-20", total_interests=27000.0, principal_amount=5000.0
        )
        exp_dates = {
        ...
        for per in periods:
            self.assertEquals(exp_dates[per.get("name")], per.get("posting_date"))

    def test_interests_with_previous_entries(self):
        create_test_recovery(total_interests=17000.0, principal_amount=15000.0)
        create_test_recovery(skip_dependencies=True, total_interests=12000.0)
        exp_amounts = dict(
            (d[0], d)
            for d in [
    ...
    def test_updates_loan_status_for_repaid(self):
        recovery = create_test_recovery(principal_amount=100000)
        recovery_status = frappe.get_value(
            "Microfinance Loan", recovery.loan, "recovery_status"
        )
    ...
    def test_gle_with_interests(self):
        recovery = create_test_recovery(
            total_interests=10000.0, principal_amount=5000.0
        )
        exp_gle = dict(
            (d[0], d)
            for d in [
...
def remove_test_recovery(loan="_Test Loan 1", keep_dependencies=False):
    recoveries = frappe.get_all("Microfinance Recovery", filters=[["loan", "in", loan]])
    for doc in recoveries:
        try:
            ...
            pass
        frappe.delete_doc(doctype="Microfinance Recovery", name=doc.name, force=True)
