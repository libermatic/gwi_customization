...
from gwi_customization.microfinance.api.loan import calculate_principal, update_amounts
...
class TestLoan(unittest.TestCase):
    ...
    def test_update_amounts(self):
        create_test_loan(
            loan_no="_Test Loan 1", loan_principal=50000.0, recovery_amount=5000.0
        )
        update_amounts(
            name="_Test Loan 1", principal_amount=60000.0, recovery_amount=6000.0
        )
        principal, recovery = frappe.get_value(
            "Microfinance Loan", "_Test Loan 1", ["loan_principal", "recovery_amount"]
        )
    ...
    def test_update_amounts_raises_when_less_than_disbursed(self):
        ...
        create_test_disbursement(
            skip_dependencies=True, loan="_Test Loan 1", amount=40000.0
        )
